...
def handler(event, context):
    try:
        ...
        item = {"PK": f"USER#{user_id}", "SK":"PROFILE", "email": email, "name": name, "role": role, "createdAt": now}
        ...
        # Tabla no encontrada, validación, permiso, etc.
        if code in {"ResourceNotFoundException","ValidationException","AccessDeniedException","UnrecognizedClientException"}:
            return _resp(500, {"error":"Error DynamoDB", "code":code, "message":e.response["Error"].get("Message")})
        # Deja que otras ClientError suban al catch general (y queden logueadas)
        raise
